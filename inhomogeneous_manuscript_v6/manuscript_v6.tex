...
with $\vq$ \emph{along} the domain wall in $S$-wave, \emph{across} domain wall in $D$-wave \blue{with nodes along the wall, and \emph{along} domain wall when the maximum gap is along the domain wall}.  
...
pointed out that nonuniform superconducting states play important role 
...
When the bound states are all concentrated at one energy 
producing a strong peak in density of states (DOS), this dramatically changes properties 
...
Another slight enhancement for $D$-wave can be seen as a circle of radius $k_f$ centered at $(0,k_f)$, 
...
the bound states and the low-energy sropagating states for near-nodal directions 
...
At lower temperatures (panels \blue{B, D and F}) the zero-field enhancement regions %that were present in $H=0$ 
